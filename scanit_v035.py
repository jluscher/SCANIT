...
siTitle = 'SCANIT for RetroSPEX [v035]'   # Program name and version
...
cmdtest = False     # print command/response queue details
...
blinkSpeed = [(50,2000), (2000,50), (200,200)]
#
...
blinkSpeed = [(50,1000), (1000,50), (200,200)]
#
# default speed is STATEstop (non-scanning)
blinkOnTime, blinkOffTime  =   blinkSpeed[STATEstop]
...
def blinkOnDelay():
    ...
    siWin.after(blinkOnTime, cmdBlinkOFF)   # delay for ON time
    return

#----
def blinkOffDelay():
    '''Set time to leave LED OFF'''
    siWin.after(blinkOffTime, cmdBlinkON)   # delay for OFF time
    return
...
def cmdHVupdate():
    ...
    global crQ, crQrsp
    if jjltest:
        print("cmdHVupdate(): update Live Data HV levels")
    #
    txt = crQrsp.split(' ')     # seperate into ["A", <hex EM>, <hex REF>]
    if jjltest:
        print('cmdHVupdate(): crQrsp: {!r}'.format(txt))
    #
    hvem  = HEXtoVOLTS(txt[1])
    hvref = HEXtoVOLTS(txt[2])
    if jjltest:
        print('cmdHVupdate(): hvem: {}, hvref: {}'.format(hvem, hvref))
    #
...
def cmdHVread():
    ...
    global crQ
    if jjltest:
        print("cmdHVread(): read converted HV levels (as Hex strings).")
    #
    crQappend( [ "A", "A # #", cmdHVupdate] )   # add to crQ for sending 
    return
...
def cmdHVconv():
    ...
    #
    if True: #jjltest:
        print("cmdHVconv(): request ADC conversion of HV voltages.")
    #
    crQappend( [ "H", "! 01", cmdHVread] )   # add to crQ for sending 
    return
...
def cmdHVon():
    ...
    crQappend( [ cmd1, cmd1, cmdHVconv] )   # add to crQ for sending
    return
...
def cmdHVoff():
    ...
    crQappend( ['D 1 0000', 'D 1 0000', cmdHVconv] ) 
    return
...
def RetroSPEXinit():
    ...
    print("#TODO - HV setup")
    #
    cmdHVoff()    # EMission and REFerence PMT voltage -> ZERO
    #
...
def PowerUp():
    ...
    if portName != 'OFFLINE':
        ...
        cmdBlinkON()    # start 'blink-cycle' (low duty cycle default)
        #
