...
siTitle = 'SCANIT for RetroSPEX [v037]'   # Program name and version
...
#  Pulse Counting Mode:   
#
varPhotonCountMode = StringVar() # 'Edge' -pulse Edge / 'Width' -pulse Width
varPhotonInversionMode = StringVar() # photon pulse signal inversion (0,1,2,3)
#TODO incorporate into RetroSPEX code and remove from Setings Pop-up Screen 
#
#==============
#  Live Data (acquired)
#
...
varLiveEMpos        = StringVar()   # Live Emission   position nm
...
#----
def doCmdA_HVupdate():
    '''Called in response to cmdA_HVread()  --  command to READ HV voltages.
...
#----
def cmdA_HVread():
    '''Called in response to cmdH_HVconv()  --  A/D conversion on HV voltages.
...
def cmdT_setIntTimer():
    ...
    #
    # integration time (seconds) is varTMinc.get()  {stringVar}
...
#----
def cmdG_setRefGain():
    '''Set "Reference Channel Gain".
    "G <n>"   # n is from set: ['0', '1', '2', '3']'''
    #
    # integration time (seconds) is varTMinc.get()  {stringVar}
    # integration time (seconds) => integer-integration-time-ms-hex-string
    cmd = 'G {}'.format(varREFdiodeG.get())
    crQappend( [cmd, cmd, nop] ) 
    return


#TODO (should be commented out once incorporated into RetroSPEX code)
#
#----
def cmdSmallN_setInversionMode():
    '''Set "Reference Photon Pulse Signal Inversion Mode - analog 'sense'".
    "n <n>"   # n is from set: ['0', '1', '2', '3']'''
    #
    cmd = 'n {}'.format(varPhotonInversionMode.get())
    crQappend( [cmd, cmd, nop] ) 
    return
  
#TODO (should be commented out once incorporated into RetroSPEX code)
#
def updatePhotonInversionMode():
    # NOTE: NOT (CURRENTLY!) SAVED - Dan wants to incorporate into code: FIXed value
    # NOT!! writeSettings()     # save updated setting
    cmdSmallN_setInversionMode()
    return

  
def updatePhotonCountMode():
    # NOTE: PhotonCountMode is only used by 'cmdP_' and 'cmdR_' commands
    writeSettings()     # save updated setting
    return
...
def dataFileMGET():
    ...
    if scanIs == EMscan:  # restore measurement variables
        ...
        #    varEMwaveEnd    = StringVar()  # Emission End   Wavelength nm
        #    varEMwaveInc    = StringVar()  # Emission Inc   Wavelength nm
        #    varTMinc        = StringVar()  # Time Inc time Sec.
        #    varEXwaveStart  = StringVar()  # Excitation Start Wavelength nm
        #
        for line in inputFileHdr:
...
#         print('readSettings()')
    ...
    # PHOTON MEASUREMENT METHOD
    varEMstepsNm.set('50')    
    #   "PhotonCountMode: Edge"  # Photon Counting on Pulse Edge (vs. Width)
    varPhotonCountMode.set('Edge') 
    #   
    #TODO (should be commented out once incorporated into RetroSPEX code)
    #   "PhotonInversionMode: Polarity"  # Photon Counting Signa Polarity (PMT & REF)
    varPhotonInversionMode.set('0')    
    #
    #
    ...
    try:
        ...
        for line in tmpFile:
            ...
            elif items[0] == "PhotonCountMode:":
                varPhotonCountMode.set(items[1])
    except:
...
#
## Spectrometer Settings Edit (default settings, etc.) for measurement
def editScannerSettings():
    '''Edit 'settings.txt' to alter default values.'''
#     if jjltest:
#         print('editScannerSettings()')
    #
    edset = Toplevel()
    ...
    #
    #
    # Photon Pulse Count Mode - spectrometer setup
    #
    #     varPhotonCountMode   = StringVar()   
    #           ('Edge' -pulse Edge / 'Width' -pulse Width)
    #
    #-------
    edsetPc = LabelFrame(edsetTop, text="Photon Count Mode." 
                        ,bg = TANBG, font=monoFont16
                        ,borderwidth=6)
    edsetPc.grid(row=2, padx=4, pady=4, sticky=EW)
    #
    #
    # Photon Count Mode  =  'Edge' / 'Width'
    #-------
    edsetPcR1 = Radiobutton(edsetPc, text="Edge", variable=varPhotonCountMode 
                           , value='Edge', bg = TANBG, font=monoFont14
                           , command=updatePhotonCountMode)
    edsetPcR1.grid(row=0, column=0, padx=4, sticky=W)
    #
    #-------
    edsetPcR2 = Radiobutton(edsetPc, text="Width", variable=varPhotonCountMode 
                           , value='Width', bg = TANBG, font=monoFont14
                           , command=updatePhotonCountMode)
    edsetPcR2.grid(row=0, column=1, padx=4, sticky=W)
    #
    #
    # Photon Pulse Signal Inversion Mode - spectrometer setup
    #
    #     varPhotonInversionMode   = StringVar() 
    #  
    # '0' - both one way, '1', '2' - alternate polarities, '3' - both the other way
    #
    #-------
    edsetINVc = LabelFrame(edsetTop, text="Photon Inversion Mode." 
                        ,bg = TANBG, font=monoFont16
                        ,borderwidth=6)
    edsetINVc.grid(row=3, padx=4, pady=4, sticky=EW)
    #
    #
    # Photon Inversion Mode  =  '0'/'1'/'2'/'3'
    #-------
    edsetINVcR0 = Radiobutton(edsetINVc, text="0", variable=varPhotonInversionMode 
                           , value='0', bg = TANBG, font=monoFont14
                           , command=updatePhotonInversionMode)
    edsetINVcR0.grid(row=0, column=0, padx=4, sticky=W)
    #
    #-------
    edsetINVcR1 = Radiobutton(edsetINVc, text="1", variable=varPhotonInversionMode 
                           , value='1', bg = TANBG, font=monoFont14
                           , command=updatePhotonInversionMode)
    edsetINVcR1.grid(row=0, column=1, padx=4, sticky=W)
    #-------
    edsetINVcR2 = Radiobutton(edsetINVc, text="2", variable=varPhotonInversionMode 
                           , value='2', bg = TANBG, font=monoFont14
                           , command=updatePhotonInversionMode)
    edsetINVcR2.grid(row=0, column=2, padx=4, sticky=W)
    #
    #-------
    edsetINVcR3 = Radiobutton(edsetINVc, text="3", variable=varPhotonInversionMode 
                           , value='3', bg = TANBG, font=monoFont14
                           , command=updatePhotonInversionMode)
    edsetINVcR3.grid(row=0, column=3, padx=4, sticky=W)
    #
    #
    # DONE  --  execute when editing is complete
    def edsetDone(x=None):
        ...
        if hvOn:
            cmdD_HVonLevels()   # set new voltage levels
            cmdH_HVconv()       # update  voltage readings
        #
        # update Reference gain setting
        cmdG_setRefGain()
        #
    ...
    bDone = Button(edsetTop, text = 'DONE', bg = TANBG, borderwidth=4 
                ,command = edsetDone
                ,activebackground=ACTIVB, font=monoFont16)
    bDone.grid(row=4,padx=4, pady=2, sticky=W)
    #
    edset.transient(siWin)
    edset.grab_set()
    siWin.wait_window(edset)
#     if jjltest:
#         print( 'editSpectrometerSettings(): Done!')
    return
...
def PowerUp():
    ...
    if portName != 'OFFLINE':
        ...
        #
        # - 'G' (gain setting)
        cmdG_setRefGain()   # set Reference Channel Gain
        #
        #TODO set to DEFAULT (at power up) mode of '0'  (pulse inversion = ??)
        #TODO ! comment out once RetroSPEX code adds a fixed setting for this
        # - 'n' (Photon Pulse - signal inversion setting)
        cmdSmallN_setInversionMode()   # set inversion to default
        #
...
#
#-------
ScannerFrame = LabelFrame(controlsFrame,text='Scanner', 
                bg = TANBG, borderwidth=4)
ScannerFrame.grid(row=12,column=0, sticky=S)
#
#
settingsIcon = PhotoImage(file='icons/icon_settings.gif')
#
#-------
settingsBtn = Button(ScannerFrame, image=settingsIcon, bg = TANBG
                    ,borderwidth = 0, command = editScannerSettings
                    ,activebackground=ACTIVB, font=monoFont14 )
settingsBtn.grid()
